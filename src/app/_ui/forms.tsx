...
export default function EntryForm({ id }: { id?: string }) {
  ...
  return <form className="entry fade-in" onSubmit={async event => {
    ...
    <div className="entry-fields"><BrandOrbit /><h1>{register ? <label htmlFor="value">Your email</label> : 'Discover your contribution'}</h1>
      {!register && <label className="entry-input-label" htmlFor="value">Your X handle</label>}
      ...
      <p id="consent" className="helper">{register ? 'We’ll email you when early access opens.' : 'Your X handle · up to 20 recent public posts.'}</p>
